...
const Card: React.FC<Props> = ({
  ...
  image
	...
	return (
		...
		>
				<div
					className={`relative inline-flex ${variant === "elegant" ? "" : " mx-auto " } ${containerClass} ${variant !== "modern" ? " mb-3.5 md:mb-4 lg:mb-5 xl:mb-6" : " xl:mb-8 md:mb-4"}`}
				>
					<div className={`flex w-full ${isNumberSize ? "aspect-square" : ""}`}>
						<Image
							...
							height={imageSize}
							quality={85}
							sizes={typeof imageSize === "number"
							  ? `(max-width: 768px) 25vw, (max-width: 1280px) 20vw, ${imageSize}px`
							  : undefined}
							layout="intrinsic"
							className={`${containerClass} ${["elegant", "modern"].includes(variant) ? "object-contain" : "object-cover bg-gray-300"}`}
						/>
				</div>
				{(effectActive === true && effectPosition === "imageOnly") && (
					<>
						<div
							className={`absolute top-0 left-0 bg-black w-full h-full opacity-0 transition-opacity duration-300 group-hover:opacity-30 ${containerClass}`}
						/>
