...
// Medusa StoreOrder -> ChawkBazar Order shape. OrderView reads:
// - tracking_number, amount (subtotal), paid_total, delivery_fee, sales_tax,
// - status.{id,name}, payment_gateway, shipping_address/billing_address,
// - customer.{email,name,contact}, products[] with pivot shape.
function mapOrderAddress(a: any): any {
  if (!a) return null;
  return {
    country: a.country_code?.toUpperCase(),
    city: a.city,
...
function mapOrderStatus(o: any): { id: number; name: string } {
  // Medusa order.status: pending | completed | archived | canceled
  // Medusa order.payment_status: not_paid | awaiting | authorized | captured | refunded | partially_refunded | partially_captured | canceled | requires_action
  const p = o?.payment_status;
  if (p === "captured" || p === "authorized") return { id: 4, name: "Paid" };
  if (p === "refunded" || p === "partially_refunded") return { id: 5, name: "Refunded" };
  if (p === "canceled") return { id: 6, name: "Cancelled" };
  if (p === "awaiting" || p === "not_paid") return { id: 2, name: "Pending Payment" };
  const s = o?.status;
  if (s === "completed") return { id: 3, name: "Completed" };
  if (s === "canceled") return { id: 6, name: "Cancelled" };
  return { id: 1, name: "Pending" };
}

export function adaptOrder(o: any): any {
  if (!o) return null;
  const shipping = mapOrderAddress(o.shipping_address);
  const billing = mapOrderAddress(o.billing_address) ?? shipping;
  const products = (o.items ?? []).map((it: any, i: number) => ({
    id: it.product_id ?? it.id,
    name: it.product_title ?? it.title,
    slug: it.product_handle,
    image: { thumbnail: it.thumbnail, original: it.thumbnail },
    unit: it.variant_title,
    price: it.unit_price,
    pivot: {
      ...
      variation_option_id: it.variant_id,
    },
  }));
  return {
    id: o.id,
    tracking_number: o.display_id ? String(o.display_id) : o.id,
    status: mapOrderStatus(o),
    payment_gateway: (o.payment_collections?.[0]?.payment_sessions?.[0]?.provider_id ?? "manual")
      .toString()
      .replace(/^pp_/, "")
      .toUpperCase(),
    amount: o.subtotal ?? 0,
    paid_total: o.total ?? 0,
    total: o.total ?? 0,
    sales_tax: o.tax_total ?? 0,
    delivery_fee: o.shipping_total ?? 0,
    discount: o.discount_total ?? 0,
    currency: o.currency_code?.toUpperCase?.() ?? "MYR",
    ...
    customer: {
      ...
      name: [o.shipping_address?.first_name, o.shipping_address?.last_name]
        .filter(Boolean)
        .join(" ") || o.email,
      contact: o.shipping_address?.phone ?? "",
    },
    customer_contact: o.shipping_address?.phone ?? "",
    shipping_address: shipping,
    billing_address: billing,
    delivery_time: o.metadata?.delivery_time,
    delivery_date: o.metadata?.delivery_date,
    messageCard: o.metadata?.messageCard,
    products,
  };
}
