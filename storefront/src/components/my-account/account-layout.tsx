...
const accountMenu = [
  ...
  {
    slug: ROUTES.ACCOUNT_EMAIL,
    name: "text-email-address",
    icon: <IoMailOutline className="w-[18px] md:w-5 h-[18px] md:h-5" />,
  },
  {
...
const AccountLayout: React.FunctionComponent<{}> = ({ children }) => {
  ...
  // Unauthenticated visitors landing on any /my-account/* route are
  // bounced to home with the login modal pre-opened. The modal-open
  // mutation is deferred to a microtask so it doesn't fire in the same
  // render cycle as the unmount triggered by router.replace — that
  // sequence used to surface as `Application error: a client-side
  // exception` from the React error boundary catching mid-render
  // setState. Defensive guard only; the primary fix is making
  // /my-account entry points auth-aware so unauthed users never reach
  // this page (see widget-link.tsx + AuthMenu).
  useEffect(() => {
    if (!isAuthorized) {
      router.replace("/");
      queueMicrotask(() => {
  ...
  }, [isAuthorized, router, setModalView, openModal]);
