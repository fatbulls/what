        ...
        {loading && !products.length ? (
          ...
          products.map((product: Product, index: number) => (
            <ProductCard
              ...
              listId={listId}
              listIndex={index}
              // First two cards are above-the-fold on most viewports —
              // mark their images as the LCP candidate so the browser
              // fetches them with high priority instead of lazily.
              imgLoading={index < 2 ? "eager" : "lazy"}
              imgFetchPriority={index === 0 ? "high" : undefined}
            />
          ))
        )}
      </div>
