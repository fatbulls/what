...
const LoginForm: React.FC<Props> = ({ layout = "modal" }) => {
  ...
  function handleForgetPassword() {
    // Forgot-password is modal-only — no dedicated page route. Even on
    // the /signin page (where layout="page"), pop the same modal so the
    ...
    <div className="overflow-hidden bg-white mx-auto rounded-lg w-full sm:w-96 md:w-450px border border-gray-300 py-5 px-5 sm:px-8">
      ...
      <div className="grid grid-cols-1 gap-4 mt-2">
        <Button
            loading={false}
            disabled={false}
            className="h-11 md:h-12 w-full mt-2.5 bg-google hover:bg-googleHover new-google"
