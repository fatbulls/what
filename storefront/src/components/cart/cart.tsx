...
export default function Cart() {
	...
	return (
		<div className="flex flex-col w-full h-full justify-between">
			...
			{!isEmpty ? (
				// `flex-1 min-h-0 overflow-hidden` on the wrapper is required:
				// OverlayScrollbars consumes the `className` prop as its
				// internal theme class, so flex-grow/h-full classes never
				// actually reached the outer scroll element. Without this,
				// `justify-between` on the parent would distribute the gap
				// and make the cart items appear vertically centered.
				...
				<motion.div
					...
					className="flex-1 min-h-0 px-5 md:px-7 pt-8 pb-5 flex justify-center flex-col items-center"
				>
					<EmptyCart />
