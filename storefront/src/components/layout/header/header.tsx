...
import { menu as fallbackMenu } from "@data/static/menus";
...
const Header: React.FC<Props> = ({ variant = "default" }) => {
  ...
  // Menu comes from the admin DB; the static fallback ensures the header
  // still renders if the /store/menu call is down or slow.
  const dbMenu = useSiteMenu("header");
  const menu = dbMenu.length ? adaptToLegacy(dbMenu) : fallbackMenu;
