...
import { mobileMenu as fallbackMobileMenu } from "@data/static/menus";
import { useSiteMenu, adaptToLegacy } from "@lib/use-site-menu";

export default function MobileMenu() {
	const [activeMenus, setActiveMenus] = useState<any>([]);
  const settings = useSettings();
  // Mirror the header menu. If we later want a separate mobile tree,
  // seed rows under menu_key="mobile" and swap the key here.
  const dbMenu = useSiteMenu("header");
  const mobileMenu = dbMenu.length ? adaptToLegacy(dbMenu) : fallbackMobileMenu;
	...
	return (
		<>
			<div className="flex flex-col justify-between w-full h-full">
				...
				{/* OverlayScrollbars consumes `className` as its theme token,
				    so `flex-grow` never applied directly — wrap in a flex-1 div. */}
				<div className="flex-1 min-h-0 overflow-hidden">
					<Scrollbar className="menu-scrollbar w-full h-full">
