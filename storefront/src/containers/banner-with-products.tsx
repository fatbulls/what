...
const BannerWithProducts: React.FC<ProductsProps> = ({
  ...
  const listId = `banner-products-${sectionHeading}`;
  const productIdsKey = useMemo(
    () =>
      Array.isArray(products?.data)
        ? products.data
            .map((product: any) => product?.id ?? product?.slug ?? "unknown")
            .join("|")
        : "",
    [products?.data]
  );
  ...
  useEffect(() => {
    ...
    trackViewItemList({ products: products.data, listName, listId });
  }, [loading, listName, listId, productIdsKey]);
