					...
					{data.map((banner: any, index: number) => (
						...
						>
							<BannerCard
								data={banner}
								effectActive={true}
								// Admin-set link (live banners) wins; legacy static
								// banners that only have a `slug` fall back to a
								// collections URL or plain /search.
								href={banner?.link || (banner?.slug ? `${ROUTES.COLLECTIONS}/${banner.slug}` : "/search")}
								priority={index <= 2}
