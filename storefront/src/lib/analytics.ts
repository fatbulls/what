...
// Analytics is configured at runtime via the admin-editable site-config
// (gtm_container_id + friends). The storefront's GTM loader only mounts if
// a container is set, so pushes below are harmless no-ops on unconfigured
// tenants. No build-time env gate.
const DEFAULT_CURRENCY = process.env.NEXT_PUBLIC_DEFAULT_CURRENCY ?? "MYR";

const canTrack = () => typeof window !== "undefined";
const hasGtm = true; // legacy flag kept for `hasAnalytics()` consumers
