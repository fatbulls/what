...
export default function HomeClient({ liveBanners }: HomeClientProps = {}) {
  ...
  // If admin has uploaded banners, use them (capped at 6 in the API and
  // again here as a safety belt). Otherwise fall back to the static
  // promotional banners shipped with the codebase.
  const heroBannerData =
    liveBanners && liveBanners.length > 0
      ? liveBanners.slice(0, 6).map((b) => ({
          id: b.id,
          title: b.title,
          link: b.link || undefined,
          image: {
            desktop: { url: b.image_url, width: 1440, height: 570 },
            mobile: {
              url: b.image_url_mobile || b.image_url,
              width: 450,
              height: 180,
            },
          },
          sizes: "(max-width: 768px) 100vw, min(60vw, 960px)",
        }))
      : promotionBanner;

  return (
    <>
      {isClient ? <BannerSliderBlock data={heroBannerData} /> : <BannerSliderSkeleton />}
      <Container>
