...
const HeaderMenu: React.FC<MenuProps> = ({ data, className }) => {
	...
	return (
		...
		{data?.map((item: any, index: number) => {
			const menuKey = [item.id, item.path, index].filter(Boolean).join("-");
			return (
				<div
				...
				>
					<Link
						href={item.path}
						className="inline-flex main-menu items-center text-sm xl:text-base text-heading px-3 xl:px-4 py-2 font-normal relative group-hover:text-black"
					>
						{t(item.label)}
