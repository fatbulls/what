...
const StripePayment: React.FC = () => {
  ...
  const availableItems = useMemo(() => {
    if (!verifiedResponse?.unavailable_products?.length) {
      return items;
    }
    return items?.filter(
      (item) => !verifiedResponse.unavailable_products.includes(item.id)
    );
  }, [items, verifiedResponse?.unavailable_products]);
  ...
  }, [baseAmount, discount, verifiedResponse]);
  ...
  const ensurePendingOrder = useCallback(async () => {
    ...
    const input: any = buildOrderInput({
      ...
      subtotal: baseAmount,
      total: paidTotal,
      verifiedTotalTax: verifiedResponse?.total_tax,
      verifiedShipping: verifiedResponse?.shipping_charge,
      couponId: checkout?.coupon?.id ? Number(checkout?.coupon?.id) : null,
      discount: discount ?? 0,
