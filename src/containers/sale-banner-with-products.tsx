...
const SaleBannerWithProducts: React.FC<ProductsProps> = ({
  ...
  const listId = `sale-banner-${sectionHeading}`;
  const productIdsKey = useMemo(
    () =>
      Array.isArray(data?.data)
        ? data.data
            .map((product: Product) => product?.id ?? product?.slug ?? "unknown")
            .join("|")
        : "",
    [data?.data]
  );
  ...
  useEffect(() => {
    ...
    trackViewItemList({ products: data.data, listName, listId });
  }, [isLoading, listName, listId, productIdsKey]);
