...
const MENU_KEY = "footer"

// Footer reuses the existing menu module with a separate menu_key.
// Top-level rows = footer widgets (their `label` shows as the widget
// title in the storefront grid). Children = the link list inside each
// widget. Sentinels supported in `href`:
//   __MY_ACCOUNT_LINK__       → auth-aware link / login modal
//   __OPEN_FORGET_PASSWORD__  → opens FORGET_PASSWORD modal
//   __WHATSAPP_URL__          → admin's social_whatsapp from site-config
const SENTINELS_HINT =
  "Path supports sentinels: __MY_ACCOUNT_LINK__ (auth-aware), __OPEN_FORGET_PASSWORD__ (modal), __WHATSAPP_URL__ (admin WhatsApp link)."
...
export default function FooterMenusPage() {
  ...
  async function saveAll() {
    ...
    try {
      ...
      toast.success("Saved footer changes")
      await reload()
    } catch (e: any) {
  ...
  async function remove(id: string) {
    if (!confirm("Delete this entry (and any children)?")) return
  ...
  const renderRow = (it: MenuItem, isChild = false) => {
    ...
    return (
      <div
        key={it.id}
        className={`grid gap-2 items-center ${isChild ? "ml-6" : ""} grid-cols-[60px_1fr_1.4fr_48px_56px]`}
      >
        <Input
          type="number"
          value={position}
        ...
        <Input
          ...
          placeholder={isChild ? "Link label" : "Widget title"}
        />
        <Input
          value={href}
          onChange={(e) => markDirty(it.id, { href: e.target.value })}
          placeholder={isChild ? "/path or sentinel (see help)" : "(parents have no href)"}
          disabled={!isChild}
        />
        <Switch
          checked={isActive}
        ...
        />
        <Button variant="danger" size="small" onClick={() => remove(it.id)}>
          Del
        </Button>
      </div>
    )
  ...
  return (
    <Container className="divide-y p-0">
      <div className="flex items-center justify-between px-6 py-4">
        <div>
          ...
          <Text className="text-ui-fg-subtle" size="small">
            Top-level rows are footer widget titles; children are the
            links inside each widget. {SENTINELS_HINT}
          </Text>
        </div>
      ...
      {loading ? (
        ...
        <>
          <div className="px-6 py-3 grid gap-2 grid-cols-[60px_1fr_1.4fr_48px_56px] text-xs text-ui-fg-subtle">
            <span>Pos</span>
            <span>Label</span>
            <span>Path / sentinel</span>
            <span>On</span>
            <span></span>
          </div>
          ...
          <div className="px-6 py-4">
            <Label size="small" weight="plus" className="mb-2 block">Add widget (top-level)</Label>
            <div className="flex gap-2">
              <Input
                placeholder="Widget title (e.g. Customer Care)"
                value={newTop.label}
              ...
              <Button
                size="small"
                onClick={async () => {
                  await create(null, newTop.label, "")
