...
export default function Collections() {
  ...
  const tagSlug = (query?.tags as string) ?? "";
  const humanReadableTag = tagSlug
    ? tagSlug.replace(/-/g, " ").replace(/\b\w/g, (char) => char.toUpperCase())
    : "Collections";
  const description = `Explore curated ${humanReadableTag} selections designed for thoughtful gifting from Because You Florist.`;

  return (
    <div className="border-t-2 border-borderBottom">
      <Seo
        pageName={humanReadableTag}
        title={`${humanReadableTag} Collection`}
        description={description}
        canonicalPath={tagSlug ? `/collections/${tagSlug}` : "/collections"}
        breadcrumbs={[
          { name: "Home", item: "/" },
          { name: "Collections", item: "/search" },
          { name: humanReadableTag, item: tagSlug ? `/collections/${tagSlug}` : "/collections" },
        ]}
        schema={{
          type: "webPage",
          data: {
